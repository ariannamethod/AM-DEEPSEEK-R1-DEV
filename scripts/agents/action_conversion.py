...
def action_conversion(
    ...
    for i, action in enumerate(actions):
        ...
        if action.function_name == "mobile.home":
            ...
            arg_name, arg_value = list(actions[i].parameters.items)[0]
            actions[i].parameters["arg_0"] = arg_value
            ...
            arg_name, arg_value = list(actions[i].parameters.items)[0]
            if arg_name != "arg_0":
                del actions[i].parameters[arg_name]
                actions[i].parameters["answer"] = arg_value
            else:
                actions[i].parameters["arg_0"] = arg_value
        ...
        # OS ACTION
            ...
            # TODO: change the coordonate from norm to pixel and remove kwargs keys
            ...
            # TODO: change the coordonate from norm to pixel
            ...
            action[i].parameters["amount"] = int(abs(arg_value * 10))

        elif action.function_name == "pyautogui.dragTo":
            actions[i].function_name = "drag"
            # TODO: Check parameters formating
